...
function jawda_project_features_build_label($arabic, $english) {
    ...
    if (function_exists('aqarand_locations_get_label')) {
        return aqarand_locations_get_label($arabic, $english, 'both', $english !== '' ? $english : $arabic);
    }
...
function jawda_project_features_get_feature_types() {
    return [
        'feature'  => jawda_project_features_build_label('ميزة', __('Feature', 'aqarand')),
        'amenity'  => jawda_project_features_build_label('وسيلة راحة', __('Amenity', 'aqarand')),
        'facility' => jawda_project_features_build_label('مرفق', __('Facility', 'aqarand')),
        'finishing' => jawda_project_features_build_label('التشطيبات', __('Finishing', 'aqarand')),
        'view' => jawda_project_features_build_label('الإطلالات', __('View', 'aqarand')),
        'orientation' => jawda_project_features_build_label('الاتجاهات', __('Orientation', 'aqarand')),
        'facade' => jawda_project_features_build_label('الواجهات / الوضع', __('Facade / Position', 'aqarand')),
        'marketing_orientation' => jawda_project_features_build_label('تسويق الاتجاهات والواجهات', __('Marketing Orientation Label', 'aqarand')),
    ];
}

function jawda_project_features_get_context_labels() {
    return [
        'projects'   => jawda_project_features_build_label('المشروعات', __('Projects', 'aqarand')),
        'properties' => jawda_project_features_build_label('الوحدات', __('Units', 'aqarand')),
        'both'       => jawda_project_features_build_label('المشروعات والوحدات', __('Projects & Units', 'aqarand')),
    ];
}
...
function jawda_project_features_get_feature_label($feature_id, $language = 'both') {
    ...
    $name_ar = isset($feature['name_ar']) ? (string) $feature['name_ar'] : '';
    $name_en = isset($feature['name_en']) ? (string) $feature['name_en'] : '';

    if (function_exists('aqarand_locations_get_label')) {
        return aqarand_locations_get_label($name_ar, $name_en, $language, $name_en !== '' ? $name_en : $name_ar);
    }
...
function jawda_get_project_features_options($context = 'projects', $allowed_types = null) {
    ...
    switch ($context) {
        case 'properties':
            ...
            $placeholder_en = __('Select unit amenities / facilities', 'aqarand');
            break;
        case 'both':
            $placeholder_ar = 'اختر العناصر المميزة';
            $placeholder_en = __('Select featured items', 'aqarand');
            break;
        default:
            $placeholder_ar = 'اختر مميزات المشروع';
            $placeholder_en = __('Select project features / amenities', 'aqarand');
            break;
    }
    ...
    if (count($allowed_types) === 1) {
        ...
        $type_placeholders = [
            'feature' => [
                'projects'   => ['ar' => 'اختر مميزات المشروع', 'en' => __('Select project features', 'aqarand')],
                'properties' => ['ar' => 'اختر مميزات الوحدة', 'en' => __('Select unit features', 'aqarand')],
            ],
            'amenity' => [
                'projects'   => ['ar' => 'اختر وسائل الراحة للمشروع', 'en' => __('Select project amenities', 'aqarand')],
                'properties' => ['ar' => 'اختر وسائل الراحة للوحدة', 'en' => __('Select unit amenities', 'aqarand')],
            ],
            'facility' => [
                'projects'   => ['ar' => 'اختر مرافق المشروع', 'en' => __('Select project facilities', 'aqarand')],
                'properties' => ['ar' => 'اختر مرافق الوحدة', 'en' => __('Select unit facilities', 'aqarand')],
            ],
        ];
    ...
    foreach ($features as $feature) {
        ...
        $label = function_exists('aqarand_locations_get_label')
            ...
            : ($language === 'en'
                ...
                : ($language === 'ar'
                    ? ($feature['name_ar'] ?? '')
                    : trim(($feature['name_ar'] ?? '') . ' / ' . ($feature['name_en'] ?? ''))
                )
            );
...
function jawda_get_feature_selection_for_post($post_id, $context, $meta_key, $language = null, $allowed_types = null) {
    ...
    if (function_exists('aqarand_locations_normalize_language')) {
        $language = aqarand_locations_normalize_language($language, $language ?: 'ar');
    }
