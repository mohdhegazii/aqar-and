...
class Jawda_Project_Features_Page {
    ...
    public function __construct($args = []) {
        $defaults = [
            ...
            'allowed_types'    => [],
            'labels'           => [
                'list_title'      => __('Featured', 'aqarand'),
                'add_new'         => __('Add New', 'aqarand'),
                'add_heading'     => __('Add Featured Item', 'aqarand'),
                'edit_heading'    => __('Edit Featured Item', 'aqarand'),
                'add_button'      => __('Add Featured Item', 'aqarand'),
                'update_button'   => __('Update', 'aqarand'),
                'success_message' => __('Featured item saved successfully.', 'aqarand'),
                'delete_success'  => __('Featured item deleted.', 'aqarand'),
                'delete_error'    => __('Failed to delete featured item.', 'aqarand'),
            ],
        ];
    ...
    private function build_item_label($item) {
        $name_ar = isset($item['name_ar']) ? (string) $item['name_ar'] : '';
    ...
    public function enqueue_assets($hook) {
        ...
        wp_localize_script(
            ...
            [
                ...
                'i18n'             => [
                    'upload' => esc_html__('Use this image', 'aqarand'),
                    'remove' => esc_html__('Remove Image', 'aqarand'),
                    'mimeError' => esc_html__('Only PNG and SVG images are allowed.', 'aqarand'),
                ],
            ]
        );
    }
    ...
    private function render_list() {
        ?>
        <div class="wrap">
            <h1 class="wp-heading-inline"><?php echo esc_html($this->get_label('list_title', __('Featured', 'aqarand'))); ?></h1>
            <?php $current_page = esc_attr($this->get_page_slug()); ?>
            <a href="?page=<?php echo $current_page; ?>&action=add" class="page-title-action"><?php echo esc_html($this->get_label('add_new', __('Add New', 'aqarand'))); ?></a>
            <?php
            ...
            ?>
            <a href="<?php echo esc_url($seed_url); ?>" class="page-title-action"><?php esc_html_e('Restore defaults', 'aqarand'); ?></a>
            <form method="post">
    ...
    private function render_form($id = 0) {
        ...
        if ($is_marketing_combination) {
            $orientation_options = $this->get_dropdown_options_for_type('orientation', __('— Select orientation —', 'aqarand'));
            $facade_options      = $this->get_dropdown_options_for_type('facade', __('— Select facade / position —', 'aqarand'));
        }

        ?>
        <div class="wrap">
            <h1><?php echo esc_html($id ? $this->get_label('edit_heading', __('Edit Featured Item', 'aqarand')) : $this->get_label('add_heading', __('Add Featured Item', 'aqarand'))); ?></h1>
            ...
            <form method="post">
                ...
                <table class="form-table">
                    <tbody>
                        <tr class="form-field form-required">
                            <th scope="row"><label for="name_en"><?php esc_html_e('Name (English)', 'aqarand'); ?></label></th>
                            <td><input name="name_en" id="name_en" type="text" value="<?php echo esc_attr($name_en); ?>" required /></td>
                        </tr>
                        <tr class="form-field form-required">
                            <th scope="row"><label for="name_ar"><?php esc_html_e('Name (Arabic)', 'aqarand'); ?></label></th>
                            <td><input name="name_ar" id="name_ar" type="text" value="<?php echo esc_attr($name_ar); ?>" required /></td>
                        </tr>
                        <tr class="form-field">
                            <th scope="row"><label for="feature_type"><?php esc_html_e('Type', 'aqarand'); ?></label></th>
                            <td>
                                <?php if ($this->forced_type === '') : ?>
                                    ...
                                    <p class="description"><?php esc_html_e('Classify whether this is a feature, amenity, or facility.', 'aqarand'); ?></p>
                                <?php else : ?>
                                    ...
                                    <p class="description"><?php esc_html_e('This screen manages a dedicated list for this type.', 'aqarand'); ?></p>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <tr class="form-field">
                            <th scope="row"><?php esc_html_e('Available For', 'aqarand'); ?></th>
                            <td>
                                <fieldset>
                                    <label>
                                        <input type="checkbox" name="feature_contexts[]" value="projects" <?php checked($context_projects); ?> />
                                        <?php echo isset($context_labels['projects']) ? esc_html($context_labels['projects']) : esc_html__('Projects', 'aqarand'); ?>
                                    </label>
                                    <br />
                                    <label>
                                        <input type="checkbox" name="feature_contexts[]" value="properties" <?php checked($context_properties); ?> />
                                        <?php echo isset($context_labels['properties']) ? esc_html($context_labels['properties']) : esc_html__('Units', 'aqarand'); ?>
                                    </label>
                                    <p class="description"><?php esc_html_e('Choose whether this item is selectable for projects, units, or both.', 'aqarand'); ?></p>
                                </fieldset>
                            </td>
                        </tr>
                        <?php if ($is_marketing_combination) : ?>
                            <tr class="form-field">
                                <th scope="row"><label for="orientation_id"><?php esc_html_e('Orientation', 'aqarand'); ?></label></th>
                                <td>
                                    ...
                                    <p class="description">
                                        ...
                                        printf(
                                            ...
                                            esc_html__('Manage orientation values from the %s screen.', 'aqarand'),
                                            sprintf(
                                                '<a href="%s">%s</a>',
                                                esc_url(admin_url('admin.php?page=jawda-project-features-orientations')),
                                                esc_html__('Orientations', 'aqarand')
                                            )
                                        );
                                        ?>
                                    </p>
                            ...
                            <tr class="form-field">
                                <th scope="row"><label for="facade_id"><?php esc_html_e('Facade / Position', 'aqarand'); ?></label></th>
                                <td>
                                    ...
                                    <p class="description">
                                        ...
                                        printf(
                                            ...
                                            esc_html__('Manage facade values from the %s screen.', 'aqarand'),
                                            sprintf(
                                                '<a href="%s">%s</a>',
                                                esc_url(admin_url('admin.php?page=jawda-project-features-facades')),
                                                esc_html__('Facades & Positions', 'aqarand')
                                            )
                                        );
                                        ?>
                                    </p>
                                    <p class="description"><?php esc_html_e('Select at least one orientation or facade to compose the marketing label.', 'aqarand'); ?></p>
                                </td>
                            </tr>
                        <?php endif; ?>
                        <tr class="form-field">
                            <th scope="row"><label for="image_id"><?php esc_html_e('Image', 'aqarand'); ?></label></th>
                            <td>
                                <div class="jawda-feature-image-field">
                                    <input type="hidden" name="image_id" id="image_id" value="<?php echo esc_attr($image_id); ?>" />
                                    <button type="button" class="button button-secondary jawda-upload-button"><?php esc_html_e('Upload Image', 'aqarand'); ?></button>
                                    <button type="button" class="button-link jawda-remove-button" <?php if (!$image_id) : ?>style="display:none;"<?php endif; ?>><?php esc_html_e('Remove Image', 'aqarand'); ?></button>
                                    <div class="jawda-image-preview" style="margin-top: 10px;">
                                        <?php if ($image_id) : ?>
                                            <?php echo wp_get_attachment_image($image_id, 'thumbnail'); ?>
                                        <?php endif; ?>
                                    </div>
                                    <p class="description"><?php esc_html_e('PNG or SVG images work best for consistent display.', 'aqarand'); ?></p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
                ...
                <?php
                $button_label = $id
                    ? $this->get_label('update_button', __('Update', 'aqarand'))
                    : $this->get_label('add_button', __('Add Featured Item', 'aqarand'));
    ...
    private function handle_seed_defaults() {
        ...
        if (!wp_verify_nonce($nonce, 'jawda_project_features_seed_defaults')) {
            wp_die(esc_html__('Security check failed.', 'aqarand'));
        }
        ...
        $this->set_notice('success', __('Default lookup data restored successfully.', 'aqarand'));
        $this->redirect_to();
    }
    ...
    public function handle_form_submission() {
        ...
        if ($name_en === '' || $name_ar === '') {
            $this->set_notice('error', __('Both Arabic and English names are required.', 'aqarand'));
            $this->redirect_to_form($id);
        }

        if (!$context_projects && !$context_properties) {
            $this->set_notice('error', __('Please choose at least one placement (projects or units).', 'aqarand'));
            $this->redirect_to_form($id);
        }

        if ($feature_type === 'marketing_orientation' && !$orientation_id && !$facade_id) {
            $this->set_notice('error', __('Please select an orientation or a facade to build the marketing label.', 'aqarand'));
            $this->redirect_to_form($id);
        }
        ...
        if ($image_id) {
            ...
            if (!in_array($mime, ['image/png', 'image/svg+xml'], true)) {
                $this->set_notice('error', __('Only PNG and SVG images are allowed.', 'aqarand'));
                $this->redirect_to_form($id);
            }
        }
        ...
        if ($result === false) {
            $this->set_notice('error', __('Database error:', 'aqarand') . ' ' . $wpdb->last_error);
            $this->redirect_to_form($id);
        }
        ...
        $this->set_notice('success', $this->get_label('success_message', __('Featured item saved successfully.', 'aqarand')));
        $this->redirect_to();
    }
    ...
    private function handle_delete() {
        ...
        if ($this->forced_type !== '') {
            ...
            if ($record_type && $record_type !== $this->forced_type) {
                $this->set_notice('error', __('This item cannot be modified from this screen.', 'aqarand'));
                $this->redirect_to();
            }
        }
        ...
        if ($deleted) {
            $this->set_notice('success', $this->get_label('delete_success', __('Featured item deleted.', 'aqarand')));
        } else {
            $this->set_notice('error', $this->get_label('delete_error', __('Failed to delete featured item.', 'aqarand')));
        }
