...
class Jawda_Project_Features_List_Table extends WP_List_Table {
    ...
    public function get_columns() {
        ...
        if ($this->forced_type === 'marketing_orientation') {
            $columns['orientation'] = __('Orientation', 'aqarand');
            $columns['facade']      = __('Facade / Position', 'aqarand');
        }

        $columns['feature_type'] = __('Type', 'aqarand');
        $columns['contexts']     = __('Available For', 'aqarand');
        $columns['image']        = __('Image', 'aqarand');
        $columns['updated']      = __('Last Updated', 'aqarand');
    ...
    protected function column_name_ar($item) {
        ...
        $actions = [
            ...
            'delete' => sprintf(
                ...
                esc_attr__('Are you sure you want to delete this feature?', 'aqarand'),
                esc_html__('Delete', 'aqarand')
            ),
        ];
    ...
    protected function column_contexts($item) {
        ...
        if (!empty($item['context_projects'])) {
            $values[] = isset($labels['projects']) ? $labels['projects'] : __('Projects', 'aqarand');
        }

        if (!empty($item['context_properties'])) {
            $values[] = isset($labels['properties']) ? $labels['properties'] : __('Units', 'aqarand');
        }

        if (!$values) {
            return esc_html__('None', 'aqarand');
        }
    ...
    protected function extra_tablenav($which) {
        ...
        if ($this->forced_type === '') {
            if (function_exists('jawda_project_features_get_feature_types')) {
                ...
                printf('<label class="screen-reader-text" for="filter-by-feature-type">%s</label>', esc_html__('Filter by type', 'aqarand'));
                echo '<select name="feature_type" id="filter-by-feature-type">';
                echo '<option value="">' . esc_html__('All types', 'aqarand') . '</option>';
        ...
        if ($context_options) {
            printf('<label class="screen-reader-text" for="filter-by-feature-context">%s</label>', esc_html__('Filter by availability', 'aqarand'));
            echo '<select name="feature_context" id="filter-by-feature-context">';
            echo '<option value="">' . esc_html__('All placements', 'aqarand') . '</option>';
    ...
    private function fetch_features($filters = []) {
        ...
        $orderby = isset($_REQUEST['orderby']) ? sanitize_text_field(wp_unslash($_REQUEST['orderby'])) : 'name_ar';
        $order   = isset($_REQUEST['order']) ? sanitize_key($_REQUEST['order']) : 'asc';

        $allowed_orderby = ['name_ar', 'name_en', 'feature_type', 'updated_at'];
