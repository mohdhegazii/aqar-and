                ...
                if (!$is_master_catalog) {
                    ...
                    if ($location_type && $location_id) {
                        if ($location_type === 'governorate') {
                            $query = $wpdb->prepare("SELECT id, {$name_col} as name FROM {$wpdb->prefix}locations_governorates WHERE id = %d", $location_id);
                            ...
                            $query = $wpdb->prepare("
                                ...
                                FROM {$wpdb->prefix}locations_cities c
                                JOIN {$wpdb->prefix}locations_governorates g ON c.governorate_id = g.id
                            ...
                            $query = $wpdb->prepare("
                                ...
                                FROM {$wpdb->prefix}locations_districts d
                                JOIN {$wpdb->prefix}locations_cities c ON d.city_id = c.id
                                JOIN {$wpdb->prefix}locations_governorates g ON c.governorate_id = g.id
