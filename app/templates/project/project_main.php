...
function get_my_project_main(){
  ...
  $property_type_ids = carbon_get_post_meta( $post_id, 'jawda_property_type_ids' );
  $unit_types = '';
  if (!empty($property_type_ids)) {
      $type_names = array_map(function($type_id) use ($post_id) {
          return jawda_get_property_type_display_name($type_id, $post_id);
      }, $property_type_ids);
      $unit_types = implode(', ', $type_names);
  }
  ...
  $location_parts = [];
  if($district_id){
      $location_parts[] = $wpdb->get_var($wpdb->prepare("SELECT {$name_col} FROM {$wpdb->prefix}locations_districts WHERE id = %d", $district_id));
  }
  if($city_id){
      $location_parts[] = $wpdb->get_var($wpdb->prepare("SELECT {$name_col} FROM {$wpdb->prefix}locations_cities WHERE id = %d", $city_id));
  }
  if($gov_id){
      $location_parts[] = $wpdb->get_var($wpdb->prepare("SELECT {$name_col} FROM {$wpdb->prefix}locations_governorates WHERE id = %d", $gov_id));
  }
			...
			<div class="row">
                ...
                <?php endif; ?>
                                        ...
                                        if (function_exists('aqarand_render_project_payment_templates')) {
                                            aqarand_render_project_payment_templates($post_id);
                                        }
                                        ?>
              ...
              <?php
                ...
                if ($hidden_count > 0) {
                    if ($services_language === 'ar') {
                        $more_label = sprintf(esc_html__('عرض المزيد (+%d)', 'aqarand'), $hidden_count);
                        $less_label = esc_html__('عرض أقل', 'aqarand');
                    } else {
                        $more_label = sprintf(esc_html__('Show more (+%d)', 'aqarand'), $hidden_count);
                        $less_label = esc_html__('Show less', 'aqarand');
                    }
                }
              ?>
                  ...
                  <?php foreach ($primary_services as $entry_index => $entry): ?>
                    <?php
                      ...
                      $display_name = $services_language === 'ar'
                          ? (isset($service['name_ar']) ? (string) $service['name_ar'] : '')
