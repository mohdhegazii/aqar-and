...
/* -----------------------------------------------------------------------------
# Catalog permalink resolver & pagination support
----------------------------------------------------------------------------- */

if ( ! function_exists( 'jawda_normalize_catalog_path' ) ) {
  function jawda_normalize_catalog_path( $path ) {
...
if ( ! function_exists( 'jawda_resolve_catalog_from_path' ) ) {
  function jawda_resolve_catalog_from_path( $path ) {
    $path = jawda_normalize_catalog_path( $path );

    if ( '' === $path ) {
      return null;
    }

    $post = get_page_by_path( $path, OBJECT, 'catalogs' );
    ...
    $candidates = [];
    $candidates[] = implode( '/', array_map( 'sanitize_title', $segments ) );
    $candidates[] = implode( '/', array_map( static function( $segment ) {
      return sanitize_title_with_dashes( $segment, '', 'save' );
    }, $segments ) );

    foreach ( array_unique( array_filter( $candidates ) ) as $candidate_path ) {
      $post = get_page_by_path( $candidate_path, OBJECT, 'catalogs' );
...
add_action( 'init', function () {
  add_rewrite_tag( '%catalog_path%', '(.+?)' );
  add_rewrite_tag( '%catalog_page%', '([0-9]+)' );

  $slug = 'catalog';
  $path_pattern = '(.+?)';

  add_rewrite_rule(
    "^{$slug}/{$path_pattern}/page/([0-9]+)/?$",
    'index.php?post_type=catalogs&catalog_path=$matches[1]&catalog_page=$matches[2]',
    'top'
  );

  add_rewrite_rule(
    "^([a-z]{2})/{$slug}/{$path_pattern}/page/([0-9]+)/?$",
    'index.php?lang=$matches[1]&post_type=catalogs&catalog_path=$matches[2]&catalog_page=$matches[3]',
    'top'
  );

  add_rewrite_rule(
    "^{$slug}/{$path_pattern}/?$",
    'index.php?post_type=catalogs&catalog_path=$matches[1]',
    'top'
  );

  add_rewrite_rule(
    "^([a-z]{2})/{$slug}/{$path_pattern}/?$",
    'index.php?lang=$matches[1]&post_type=catalogs&catalog_path=$matches[2]',
    'top'
  );
}, 20 );
...
add_action( 'parse_request', function ( $wp ) {
  ...
  if ( empty( $wp->query_vars['catalog_path'] ) ) {
    return;
  }

  $catalog = jawda_resolve_catalog_from_path( $wp->query_vars['catalog_path'] );

  if ( ! $catalog ) {
    // Allow WordPress to handle the 404 normally when we cannot resolve the path.
    return;
  }

  $page_number = 1;

  if ( ! empty( $wp->query_vars['catalog_page'] ) ) {
    $page_number = (int) $wp->query_vars['catalog_page'];
  } elseif ( ! empty( $wp->query_vars['paged'] ) ) {
    $page_number = (int) $wp->query_vars['paged'];
  }

  $page_number = max( 1, $page_number );

  $wp->query_vars['post_type'] = 'catalogs';
  $wp->query_vars['p']         = $catalog->ID;
  $wp->query_vars['name']      = $catalog->post_name;
  $wp->query_vars['paged']     = $page_number;
  $wp->query_vars['jawda_catalog_paged'] = $page_number;

  unset( $wp->query_vars['page'] );
  unset( $wp->query_vars['catalog_page'] );
  unset( $wp->query_vars['catalog_path'] );
...
add_action( 'pre_get_posts', function( $query ) {
  ...
  if ( 'catalogs' !== $query->get( 'post_type' ) || ! $query->get( 'p' ) ) {
    return;
  }

  $page_number = max( 1, (int) $query->get( 'jawda_catalog_paged' ), (int) $query->get( 'paged' ) );
...
add_filter( 'redirect_canonical', function( $redirect_url, $requested_url ) {
  if ( preg_match( '#/(?:[a-z]{2}/)?catalog/.+/page/[0-9]+/?$#', $requested_url ) ) {
    return false;
  }
...
add_filter( 'pre_handle_404', function( $preempt, $wp_query ) {
  ...
  if ( 'catalogs' !== ( $wp_query->query_vars['post_type'] ?? '' ) || empty( $wp_query->query_vars['p'] ) ) {
    return $preempt;
  }

  $page_number = max( 1, (int) ( $wp_query->query_vars['jawda_catalog_paged'] ?? 0 ), (int) $wp_query->get( 'paged' ) );
...
function jawda_snooze_developer_notice_ajax() {
    check_ajax_referer( 'jawda_snooze_nonce', 'security' );
