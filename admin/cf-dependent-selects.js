(function($){
  ...
  function showError(msg) {
    ...
    var $box = $('.cf-dep-errors-box #cf-dep-errors');
    if ($box.length) {
  ...
  $(function(){
    relocateProjectLocationMetaBox();
    bindMapController();

    var i18n = CF_DEP.i18n || {};
    var $gov      = $('.cf-dep-governorate select');
    var $city     = $('.cf-dep-city select');
    var $district = $('.cf-dep-district select');

    if (!$gov.length || !$city.length || !$district.length) {
      console.warn('[CF_DEP] One or more select fields could not be found. Check field classes: .cf-dep-governorate, .cf-dep-city, .cf-dep-district');
      return;
    }

    var hydration = {
      city: $city.attr('data-selected') || '',
      district: $district.attr('data-selected') || ''
    };
    ...
    $gov.on('change', function(event, reason) {
      ...
      .done(function(res) {
        if (!res.success || !res.data || typeof res.data.options === 'undefined') {
          showError('Invalid response from server when fetching cities.');
          fill($city, { '': { label: i18n.error_loading_cities || '— Error —' } });
          return;
        }

        if (Object.keys(res.data.options).length <= 1) {
          showError(i18n.no_cities_found || 'No cities found for this governorate.');
        }
    ...
    $city.on('change', function(event, reason) {
      ...
      .done(function(res) {
        if (!res.success || !res.data || typeof res.data.options === 'undefined') {
          showError('Invalid response from server when fetching districts.');
          fill($district, { '': { label: i18n.error_loading_districts || '— Error —' } });
          return;
        }

        if (Object.keys(res.data.options).length <= 1) {
          showError(i18n.no_districts_found || 'No districts found for this city.');
        }
